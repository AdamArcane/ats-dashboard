...
class Widget_Base_Helper {
	...
	public function get_original() {
		...
		if ( ! isset( $wp_meta_boxes['dashboard'] ) || ! is_array( $wp_meta_boxes['dashboard'] ) ) {
			...
			remove_action( 'wp_dashboard_setup', array( Widget_Base_Output::get_instance(), 'remove_default_dashboard_widgets' ), 100 );

			do_action( 'ats_before_wp_dashboard_setup' );

			wp_dashboard_setup();

			add_action( 'wp_dashboard_setup', array( Widget_Base_Output::get_instance(), 'remove_default_dashboard_widgets' ), 100 );

			do_action( 'ats_after_wp_dashboard_setup' );
	...
	public function get_3rd_party() {
		...
		foreach ( $widgets as $key => $value ) {
			if ( 0 === strpos( $key, 'ms-ats' ) ) {
				$ats_widgets[ $key ] = $value;
			}
		}

		$widgets = array_diff_key( $widgets, $ats_widgets, $default_widgets );
	...
	public function get_allowed_tags() {
		...
		return apply_filters( 'ats_html_widget_allowed_tags', $allowed_tags );
