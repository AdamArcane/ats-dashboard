...
const listenFormPositionFieldChange = () => {
	wp.customize("ats_login[form_position]", function (setting: any) {
		...
		const formBorderStyleStyleTag = document.querySelector(
			'[data-listen-value="ats_login[form_border_style]"]'
		) as HTMLStyleElement;
		...
		setting.bind(function (val: string) {
			const formBgColor = wp.customize("ats_login[form_bg_color]").get();
			...
			if (val === "default") {
				...
				let formBorderWidth = wp
					.customize("ats_login[form_border_width]")
					.get();
				...
				let formBorderStyle = wp
					.customize("ats_login[form_border_style]")
					.get();
				...
				let formBorderColor = wp
					.customize("ats_login[form_border_color]")
					.get();
