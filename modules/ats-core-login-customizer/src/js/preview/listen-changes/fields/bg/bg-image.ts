...
const listenBgImageFieldChange = (opts: BgFieldsOpts) => {
	...
	wp.customize("ats_login[" + keyPrefix + "bg_image]", function (setting: any) {
		...
		const bgImageStyleTag = document.querySelector(
			'[data-listen-value="ats_login[' + keyPrefix + 'bg_image]"]'
		) as HTMLStyleElement;
		...
		setting.bind(function (val: string) {
			...
			if (val) {
				...
				if (!keyPrefix) {
					const enableBgOverlay = wp
						.customize("ats_login[enable_bg_overlay_color]")
						.get();
			...
			const bgRepeat = wp
				.customize("ats_login[" + keyPrefix + "bg_repeat]")
				.get();

			writeStyle({
				styleEl: '[data-listen-value="ats_login[' + keyPrefix + 'bg_repeat]"]',
				cssSelector: cssSelector,
			...
			writeBgPositionStyle({
				styleEl:
					'[data-listen-value="ats_login[' + keyPrefix + 'bg_position]"]',
				keyPrefix: keyPrefix,
				cssSelector: cssSelector,
			...
			writeStyle({
				styleEl: '[data-listen-value="ats_login[' + keyPrefix + 'bg_size]"]',
				cssSelector: cssSelector,
