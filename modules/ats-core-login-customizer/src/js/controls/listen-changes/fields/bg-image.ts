...
const listenBgImageFieldChange = (opts: OptsWithKeyPrefix) => {
	...
	wp.customize("ats_login[" + keyPrefix + "bg_image]", function (setting: any) {
		const bgImageField = document.querySelector(
			'[data-control-name="ats_login[' + keyPrefix + 'bg_image]"]'
		) as HTMLElement;

		setting.bind(function (val: string) {
			const bgPosition = wp
				.customize("ats_login[" + keyPrefix + "bg_position]")
				.get();

			const bgSize = wp.customize("ats_login[" + keyPrefix + "bg_size]").get();

			if (val) {
				bgImageField.classList.remove("is-empty");

				wp.customize
					.control("ats_login[" + keyPrefix + "bg_position]")
					.activate();

				handleBgCustomPostion(keyPrefix, bgPosition);

				wp.customize.control("ats_login[" + keyPrefix + "bg_size]").activate();

				handleBgCustomSize(keyPrefix, bgSize);

				wp.customize
					.control("ats_login[" + keyPrefix + "bg_repeat]")
					.activate();

				// The overlay feature is only for bg_image, not for for form_bg_image.
				if (!keyPrefix) {
					wp.customize.control("ats_login[enable_bg_overlay_color]").activate();

					if (wp.customize("ats_login[enable_bg_overlay_color]").get()) {
				...
				wp.customize
					.control("ats_login[" + keyPrefix + "bg_custom_position]")
					.deactivate();

				wp.customize
					.control("ats_login[" + keyPrefix + "bg_size]")
					.deactivate();

				wp.customize
					.control("ats_login[" + keyPrefix + "bg_custom_size]")
					.deactivate();
				...
				if (!keyPrefix) {
					wp.customize
						.control("ats_login[enable_bg_overlay_color]")
						.deactivate();
