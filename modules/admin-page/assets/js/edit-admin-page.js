(function ($) {
	...
	function setupIconPicker() {
		const $iconSelect = $('[name="ats_menu_icon"]');
	...
	function setupWidgetRoles() {
		var fields = findHtmlEls(".ats-widget-roles-field");
	...
	function setupCodeMirror() {
		 ...
		 * Because ats Pro version <=3.8.0 doesn't have the `ats-codemirror` class.
		 */
		const jsFields = findHtmlEls(".ats-custom-js");

		jsFields.forEach(function (field) {
			field.classList.add("ats-codemirror");
			field.setAttribute("data-content-mode", "js");
		});

		const fields = findHtmlEls(".ats-codemirror");
		if (!fields.length) return;
	...
	function setupContentType() {
		const select = findHtmlEl('[name="ats_content_type"]');
	...
	function contentTypeSwitch(value) {
		const htmlEditor = findHtmlEl("#ats-html-metabox");
