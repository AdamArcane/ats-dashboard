...
return function () {
	...
	if ( ! wp_verify_nonce( $nonce, 'ats_admin_page_' . $post_id . '_change_active_status' ) ) {
	...
	if ( $is_active ) {
		...
		wp_send_json_success(
			wp_sprintf(
				'"%s" ' . __( 'page has been activated.', 'ats-dashboard' ),
				$page->post_title
			)
		);
	} else {
		...
		wp_send_json_success(
			wp_sprintf(
				'"%s" ' . __( 'page has been de-activated.', 'ats-dashboard' ),
				$page->post_title
			)
		);
	}
