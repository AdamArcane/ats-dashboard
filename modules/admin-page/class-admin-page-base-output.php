 ...
 */
class Admin_Page_Base_Output extends Base_Output {
	...
	public function get_posts( $menu_type ) {
		...
		if ( ! empty( $posts ) && 'parent' === $menu_type && apply_filters( 'ats_font_awesome', true ) ) {
			// Font Awesome.
		...
		foreach ( $posts as &$post ) {
			...
			$post->menu_type     = get_post_meta( $post_id, 'ats_menu_type', true );
			$post->menu_parent   = get_post_meta( $post_id, 'ats_menu_parent', true );
			$post->menu_order    = get_post_meta( $post_id, 'ats_menu_order', true );
			$post->menu_order    = $post->menu_order ? absint( $post->menu_order ) : 10;
			$post->icon_class    = get_post_meta( $post_id, 'ats_menu_icon', true );
			$post->custom_css    = get_post_meta( $post_id, 'ats_custom_css', true );
			$post->custom_js     = get_post_meta( $post->ID, 'ats_custom_js', true );
			$post->content_type  = get_post_meta( $post_id, 'ats_content_type', true );
			$post->html_content  = get_post_meta( $post_id, 'ats_html_content', true );
			$post->allowed_roles = get_post_meta( $post->ID, 'ats_allowed_roles', true );
			...
			$post->remove_page_title    = (int) get_post_meta( $post_id, 'ats_remove_page_title', true );
			$post->remove_page_margin   = (int) get_post_meta( $post_id, 'ats_remove_page_margin', true );
			$post->remove_admin_notices = get_post_meta( $post_id, 'ats_remove_admin_notices', true );
		}
	...
	public function prepare_menu( $posts, $from_multisite = false ) {
		...
		$user_roles = apply_filters( 'ats_admin_page_user_roles', $user_roles );

		foreach ( $posts as $post ) {
			$is_allowed = true;
			$is_allowed = apply_filters( 'ats_admin_page_role_is_allowed', $is_allowed, $user_roles, $post );
	...
	public function add_menu( $post, $from_multisite = false ) {
		...
		add_action(
			...
			function () use ( $post, $screen_id ) {
				...
				do_action( 'ats_admin_page_prepare_output', $post, $screen_id );

				add_action(
					'admin_print_footer_scripts',
					function () use ( $post ) {
						do_action( 'ats_admin_page_scripts', $post );
					}
				);
			}
		);
	...
	public function add_menu_icon( $menu_slug, $icon_class ) {
		...
		<style>
		#toplevel_page_ats_page_<?php echo esc_attr( $menu_slug ); ?> .wp-menu-image::before {
	...
	public function restrict_frontend() {
		if ( is_user_logged_in() || ! is_singular( 'ats_admin_page' ) ) {
			return;
		}
