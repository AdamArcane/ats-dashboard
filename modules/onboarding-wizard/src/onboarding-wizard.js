...
(function ($) {
	// Cache DOM elements.
	const page = findHtmlEl(".ats-onboarding-wizard-page");
	...
	const skipDiscount = findHtmlEl(".ats-onboarding-wizard-skip-discount a");
	...
	const loginRedirectCheckbox = findInputEl("#ats_modules__login_redirect");
	const exploreSettingsElement = findHtmlEl(
		"#onboarding-wizard-explore-settings"
	);
	const dotsWrapper = findHtmlEl(".onboarding-wizard-heatbox .ats-dots");
	const roleDropdown = findHtmlEl("#remove_by_roles");

	const atsOnboardingWizard = window.atsOnboardingWizard;
	...
	function setupSlider() {
		slider = window.tns({
			container: ".ats-onboarding-wizard-slides",
			items: 1,
			loop: false,
			autoHeight: true,
			controls: false,
	...
	function onSkipButtonClick() {
		switch (currentSlide) {
			...
			case "custom_login_url":
				...
				break;
			case "subscription":
				window.location.href = atsOnboardingWizard?.adminUrl ?? "";
				break;
		}
	}
	...
	function onSubscribeButtonClick() {
		...
		const name = findInputEl("#ats-subscription-name")?.value ?? "";
		const email = findInputEl("#ats-subscription-email")?.value ?? "";
		const data = {
			action: "ats_onboarding_wizard_subscribe",
			nonce: atsOnboardingWizard?.nonces.subscribe,
			name: name,
			email: email,
		};
	...
	function checkLoginRedirectCheckbox() {
		const sliderNav = findHtmlEl(
			".onboarding-wizard-heatbox .ats-dots .tns-nav"
		);
		if (!sliderNav) return;
	...
	function getSaveData() {
		...
		let data = {
			action: "ats_onboarding_wizard_save_modules",
			nonce: atsOnboardingWizard?.nonces.saveModules,
			modules: getSelectedModules(),
		};

		if (target?.classList.contains("js-save-widgets")) {
			data = {
				action: "ats_onboarding_wizard_save_widgets",
				nonce: atsOnboardingWizard?.nonces.saveWidgets,
				widgets: getSelectedWidgets(),
			};
		} else if (target?.classList.contains("js-save-general-settings")) {
			data = {
				action: "ats_onboarding_wizard_save_general_settings",
				nonce: atsOnboardingWizard?.nonces.saveGeneralSettings,
				settings: getGeneralSettings(),
				selected_roles: finalSelectedRoles,
			};
		} else if (target?.classList.contains("js-save-custom-login-url")) {
			data = {
				action: "ats_onboarding_wizard_save_custom_login_url",
				nonce: atsOnboardingWizard?.nonces.saveCustomLoginUrl,
	...
	function getSelectedModules() {
		...
		checkboxes.forEach(function (checkbox) {
			var module = checkbox.id.replace("ats_modules__", "");
	...
	function getSelectedWidgets() {
		...
		const checkboxes = findInputEls(
			'.ats-widgets-slide .widget-toggle input[type="checkbox"]'
		);
		if (!checkboxes.length) return [];
		...
		checkboxes.forEach(function (checkbox) {
			var widget = checkbox.id.replace("ats_widgets__", "");
	...
	function getGeneralSettings() {
		...
		var checkboxes = findInputEls(
			'.ats-general-settings-slide .setting-toggle input[type="checkbox"]'
		);
		if (!checkboxes.length) return [];
		...
		checkboxes.forEach(function (checkbox) {
			var setting = checkbox.id.replace("ats_settings__", "");
	...
	function getCustomLoginUrl() {
		...
		const customLoginUrlField = findInputEl("#ats_login_redirect");
	...
	function ajaxPost(data, successCallback, button) {
		...
		$.post(atsOnboardingWizard?.ajaxUrl ?? "", data, function (response) {
			doingAjax = false;
			stopLoading(button);
	...
	function onSkipDiscountClick(e) {
		...
		const data = {
			action: "ats_onboarding_wizard_skip_discount",
			nonce: atsOnboardingWizard?.nonces.skipDiscount,
		};
	...
	function hideSetupWizardSubmenuItem() {
		...
		for (let i = 0; i < submenuItems.length; i++) {
			...
			if (link.href.includes("page=ats_onboarding_wizard")) {
