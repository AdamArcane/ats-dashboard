...
class Save_Widgets {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_ats_onboarding_wizard_save_widgets', [ $this, 'handler' ] );
	...
	private function validate() {
		...
		if ( ! wp_verify_nonce( $nonce, 'ats_onboarding_wizard_save_widgets_nonce' ) ) {
		...
		if ( isset( $_POST['widgets'] ) && ! is_array( $_POST['widgets'] ) ) {
			wp_send_json_error( __( 'Widgets must be an array', 'ats-dashboard' ), 401 );
		}
	...
	private function save() {
		...
		wp_send_json_success( __( 'Widgets saved', 'ats-dashboard' ) );
