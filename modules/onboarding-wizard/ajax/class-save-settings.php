...
class Save_Settings {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_ats_onboarding_wizard_save_general_settings', [ $this, 'handler' ] );
	...
	private function validate() {
		...
		if ( ! wp_verify_nonce( $nonce, 'ats_onboarding_wizard_save_general_settings_nonce' ) ) {
			wp_send_json_error( __( 'Invalid token', 'ats-dashboard' ), 401 );
		}

		// If settings exists, it must be an array.
		if ( isset( $_POST['settings'] ) && ! is_array( $_POST['settings'] ) ) {
			wp_send_json_error( __( 'Settings must be an array', 'ats-dashboard' ), 400 );
		}
		...
		if ( isset( $_POST['selected_roles'] ) && ! is_array( $_POST['selected_roles'] ) ) {
			wp_send_json_error( __( 'Selected roles must be an array', 'ats-dashboard' ), 400 );
		}
	...
	private function save() {
		...
		foreach ( $this->available_settings as $available_setting ) {
			if ( in_array( $available_setting, $this->settings, true ) ) {
				if ( 'remove_admin_bar' === $available_setting ) {
					$ats_settings[ $available_setting ] = $this->selected_roles;
				} else {
					$ats_settings[ $available_setting ] = 1;
				}

				continue;
			}

			// If the setting is not selected, remove it from the ats settings.
			if ( isset( $ats_settings[ $available_setting ] ) ) {
				unset( $ats_settings[ $available_setting ] );
			}
		}
		...
		wp_send_json_success( __( 'Settings saved', 'ats-dashboard' ) );
