...
class Save_Modules {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_ats_onboarding_wizard_save_modules', [ $this, 'handler' ] );
	...
	private function validate() {
		...
		if ( ! wp_verify_nonce( $nonce, 'ats_onboarding_wizard_save_modules_nonce' ) ) {
		...
		if ( empty( $_POST['modules'] ) || ! is_array( $_POST['modules'] ) ) {
			wp_send_json_error( __( 'No modules selected', 'ats-dashboard' ), 401 );
		}
	...
	private function save() {
		...
		update_option( 'ats_modules', $module_statuses );

		// Get the login_redirect value from ats_modules.
		...
		wp_send_json_success( [
			'message'        => __( 'Modules saved', 'ats-dashboard' ),
