...
class Subscribe {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_ats_onboarding_wizard_subscribe', [ $this, 'handler' ] );
	...
	private function validate() {
		...
		if ( ! wp_verify_nonce( $nonce, 'ats_onboarding_wizard_subscribe_nonce' ) ) {
		...
		if ( empty( $this->name ) ) {
			wp_send_json_error( __( 'Name field is empty', 'ats-dashboard' ), 401 );
		}

		$this->email = isset( $_POST['email'] ) ? sanitize_text_field( wp_unslash( $_POST['email'] ) ) : '';

		if ( empty( $this->email ) ) {
			wp_send_json_error( __( 'Email field is empty', 'ats-dashboard' ), 401 );
		}
	}
	...
	private function subscribe() {
		...
		$proxy_url = 'https://ats-dashboard.io/wp-json/mailerlite-proxy/v1/subscribe';
		$group_id  = '112818510';
		...
		if ( $response_code >= 200 && $response_code < 300 ) {
			update_option( 'ats_onboarding_wizard_completed', true );
			wp_send_json_success( __( 'Subscription done', 'ats-dashboard' ) );
		}

		$error_data = json_decode( $response_body, true );
		$error_message = isset( $error_data['message'] ) ? $error_data['message'] : __( 'Subscription failed', 'ats-dashboard' );
