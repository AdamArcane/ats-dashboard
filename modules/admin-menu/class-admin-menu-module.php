...
class Admin_Menu_Module extends Base_Module {
	...
	public function non_blueprint_notice() {
		...
		<div class="atsui ats-notice-metabox is-warning">
			...
			<div class="atsui-content">
				...
				$description .= __( 'To inherit the blueprint configuration again for the Admin Menu Editor, please <strong>reset the menu</strong> (button below).', 'ats-dashboard' );
				?>
	...
	/**
	 * Return the current admin menu for the builder.
	 *
	 * @param object $ajax_handler The menu AJAX handler.
	 * @param string $role Unused - kept so this action's signature stays compatible
	 *                     with the multisite module's own hook on the same action.
	 */
	public function get_admin_menu( $ajax_handler, $role = 'default' ) {

		$ajax_handler->load_menu();
		wp_send_json_success( $ajax_handler->format_response() );

	}

	/**
	 * Save the latest admin menu, used to keep the builder in sync with menu
	 * items registered by other plugins even between page loads.
	 */
